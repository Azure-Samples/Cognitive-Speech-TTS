...
static size_t UHTTP_ResponseHandler(void *ptr, size_t size, size_t nmemb, void *userdata)
{
    ...
    data->fnChunkReplyCallback(data, data->userCtx, ptr, size * nmemb, data->httpCode, data->respHeader, false);
...
)
{
    ...
    result = PrepareRequest(data->hHttp, httpAPIRequestType, relativePath, httpHeadersHandle, content, contentLength, addChunk);
...
HTTP_CLIENT_HANDLE http_client_create(
    ...
    ON_HTTP_REPLY_RECV_CALLBACK on_http_reply_recv, 
    ...
    HTTP_HEADERS_HANDLE httpHeadersHandle, const unsigned char* content, size_t contentLength)
{
    ...
    if (result == HTTP_CLIENT_OK)
    {
        ...
        /* Execute request */
        int still_running;
        while (CURLM_OK == curl_multi_perform(data->multi, &still_running) && still_running)
        {
            ;
        }
		...
		if (!data->httpCode)
        {
            ...
            data->fnReplyCallback(data, data->userCtx, NULL, 0, data->httpCode, data->respHeader);
        }
    }
...
void http_client_dowork(HTTP_CLIENT_HANDLE handle)
{
    ...
    else
    {
            ...
            !still_running)
        {
            ...
            if (data->httpCode)
            {
				data->fnChunkReplyCallback(data, data->userCtx, NULL, 0, data->httpCode, data->respHeader, true);
            }
        }
    }
}
