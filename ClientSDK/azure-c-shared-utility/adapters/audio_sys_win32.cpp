...
)
{
    ...
    events[1] = pAudio->hRenderThreadExit;
    ...
    void* pContext)
{
    ...
    audioData->hRenderThread = CreateThread(0, 0, renderThreadProc, pAudio, 0, nullptr);
...
AUDIO_SYS_HANDLE audio_create()
{
    ...
    if (result)
    {
        ...
        result->hCaptureThreadExit = CreateEvent(0, FALSE, FALSE, nullptr);
        if (nullptr == result->hCaptureThreadExit) { hr = E_FAIL; goto Exit; }
        ...
        result->hRenderThreadExit = CreateEvent(0, FALSE, FALSE, nullptr);
        if (nullptr == result->hRenderThreadExit) { hr = E_FAIL; goto Exit; }
    }
...
Exit:
    ...
    if (FAILED(hr))
    {
        ...
        CloseHandle(result->hBufferReady);
        CloseHandle(result->hCaptureThreadExit);

        SAFE_RELEASE(result->pAudioOutputDevice);
        CloseHandle(result->hRenderThreadExit);
...
void audio_destroy(AUDIO_SYS_HANDLE handle)
{
    if (handle != NULL)
    {
        ...
        CloseHandle(audioData->hBufferReady);
        CloseHandle(audioData->hCaptureThreadExit);

        SAFE_RELEASE(audioData->pAudioOutputDevice);
        CloseHandle(audioData->hRenderThreadExit);
...
AUDIO_RESULT audio_output_stop(AUDIO_SYS_HANDLE handle)
{
    ...
    if (handle != NULL)
    {
        ...
        else
        {
            ...
            SetEvent(audioData->hRenderThreadExit);
...
)
{
    ...
    HANDLE allEvents[] = { pInput->hCaptureThreadExit, pInput->hBufferReady };
...
AUDIO_RESULT audio_input_stop(AUDIO_SYS_HANDLE handle)
{
    ...
    if (handle != NULL)
    {
        ...
        else
        {
            ...
            // Exit our proc thread.
            SetEvent(audioData->hCaptureThreadExit);
