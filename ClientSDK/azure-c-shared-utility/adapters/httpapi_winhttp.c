...
HTTPAPI_RESULT HTTPAPI_Request(HTTP_HANDLE handle, HTTPAPI_REQUEST_TYPE requestType, const char* relativePath,
	HTTP_HEADERS_HANDLE httpHeadersHandle, const unsigned char* content,
							...
							else
							{
								...
								else
								{
									...
									if (httpRequestHandle == NULL) {
										(void)WinHttpCloseHandle(requestHandle);
										result = HTTPAPI_INVALID_ARG;
										LogError("httpRequestHandle was NULL (result = %s)", ENUM_TO_STRING(HTTPAPI_RESULT, result));
									}
									else {
...
HTTPAPI_RESULT HTTPAPI_Rsponse(unsigned int* statusCode,
	HTTP_HEADERS_HANDLE responseHeadersHandle, BUFFER_HANDLE responseContent,
	REQUESTHANDLE httpRequestHandle, HTTPAPI_READEVENT_TYPE* ReadEvery)
{
	...
	if (httpRequestHandle == NULL || *httpRequestHandle == NULL)
	{
		result = HTTPAPI_OPEN_REQUEST_FAILED;
		LogErrorWinHTTPWithGetLastErrorAsString("WinHttpOpenRequest failed (result = %s).", ENUM_TO_STRING(HTTPAPI_RESULT, result));
	}
	...
	{
		HINTERNET requestHandle = *httpRequestHandle;

		DWORD responseBytesAvailable;

		if (*ReadEvery == HTTPAPI_START_READ) 
		{
			if (!WinHttpReceiveResponse(
				requestHandle,
				0))
			...
			else
			{
				...
				{
					BUFFER_HANDLE useToReadResponse = (responseContent != NULL) ? responseContent : BUFFER_new();
					...
					else
					{
						*ReadEvery = HTTPAPI_GOON_READ;
						...
						{
							if (BUFFER_enlarge(useToReadResponse, responseBytesAvailable) != 0)
							{
								(void)WinHttpCloseHandle(requestHandle);
								result = HTTPAPI_ERROR;
								LogError("(result = %s)", ENUM_TO_STRING(HTTPAPI_RESULT, result));
							...
							else
							{
								...
								if (BUFFER_content(useToReadResponse, &bufferContent) != 0)
								{
									(void)WinHttpCloseHandle(requestHandle);
									result = HTTPAPI_ERROR;
									LogError("(result = %s)", ENUM_TO_STRING(HTTPAPI_RESULT, result));
								...
								else if (BUFFER_size(useToReadResponse, &bufferSize) != 0)
								{
									(void)WinHttpCloseHandle(requestHandle);
									result = HTTPAPI_ERROR;
									LogError("(result = %s)", ENUM_TO_STRING(HTTPAPI_RESULT, result));
		...
		else if (*ReadEvery == HTTPAPI_GOON_READ)
		{
			BUFFER_HANDLE useToReadResponse = (responseContent != NULL) ? responseContent : BUFFER_new();
		...
		{
			result = HTTPAPI_INVALID_ARG;
			LogError("HTTPAPI_READEVENT_TYPE was NULL (result = %s)", ENUM_TO_STRING(HTTPAPI_RESULT, result));
		}
	}
	return result;
}
...
static int add_pending_io(ASYNCHTTPREQUEST* ws_io_instance, const unsigned char* buffer, size_t bufferSize, int flags, ON_SEND_COMPLETE on_send_complete, void* callback_context)
{
    int result;
    ...
    {
        pending_socket_io->bytes = (unsigned char*)malloc(bufferSize);
...
typedef struct HTTP_CLIENT_HANDLE_DATA_TAG
{
    ...
    ON_HTTP_REPLY_RECV_CALLBACK fnReplyCallback;
...
HTTP_CLIENT_HANDLE http_client_create(XIO_HANDLE xioHandle, ON_HTTP_CONNECTED_CALLBACK onConnect, ON_HTTP_REPLY_RECV_CALLBACK on_http_reply_recv, ON_HTTP_ERROR_CALLBACK on_http_error, ON_HTTP_CHUNK_REPLY_CALLBACK on_chunk_reply, void* callback_ctx)
{
    ...
    else
    {
        ...
        else
        {
            ...
            result->fnReplyCallback = on_http_reply_recv;
...
HTTP_CLIENT_RESULT http_client_send_chunk_request(HTTP_CLIENT_HANDLE handle, const unsigned char* content, size_t contentLength)
{
    HTTP_CLIENT_RESULT result;
    if (handle == NULL)
    {
    ...
    else
    {
        ...
        result = add_pending_io(&data->request, (unsigned char*)chunk, len, 0, NULL, NULL);
        if (0 != contentLength)
        {
            result = add_pending_io(&data->request, content, contentLength, 0, NULL, NULL);
        }

        result = add_pending_io(&data->request, (unsigned char*)"\r\n", 2, 0, NULL, NULL);
    }
    return result;
}
        ...
        case UHTTP_RECV_COMPLETE:
            {
                ...
                if (data->dwContentLength != CONTENTLEN_CHUNKED)
                {
                    data->fnReplyCallback(handle, data->userCtx, BUFFER_u_char(data->request.rxbuff), data->bytesReceived, data->request.dwStatusCode, data->respHeader);
                    done = true;
                }
                else
                {
...
HTTP_CLIENT_RESULT http_client_set_proxy_auth(HTTP_CLIENT_HANDLE handle, const char* username, const char* password)
{
    HTTP_CLIENT_RESULT result;
    (void)username;
    (void)password;
    ...
    WSIO_INSTANCE* wsio_instance)
{
    ...
    if (ret)
    {
        LogErrorWinHTTPWithGetLastErrorAsString("WinHttpWebSocketReceive() failed.");
        return -1;
    }
    ...
    )
{
    ...
    case WINHTTP_CALLBACK_STATUS_REQUEST_ERROR:
    {
        ...
        LogInfo("WSCB: WINHTTP_CALLBACK_STATUS_REQUEST_ERROR dwError %d dwResult %d\n", p->dwError, p->dwResult);
        LogErrorWinHTTPWithGetLastErrorAsString("WINHTTP_CALLBACK_STATUS_REQUEST_ERROR() failed.");
        WSError(wsio_instance);
        return;
    }
...
CONCRETE_IO_HANDLE wsio_create(void* io_create_parameters)
{
    ...
    else
    {
        ...
        if (result != NULL)
        {
            ...
            else if (BUFFER_enlarge(result->request.rxbuff, WS_GROW_SIZE))
            {
                LogErrorWinHTTPWithGetLastErrorAsString("BUFFER_enlarge() failed.");
                free(result);
                result = NULL;
            }
            else
...
int wsio_close(CONCRETE_IO_HANDLE ws_io, ON_IO_CLOSE_COMPLETE on_io_close_complete, void* on_io_close_complete_context)
{
    ...
    {
        WSIO_INSTANCE* wsio_instance = (WSIO_INSTANCE*)ws_io;
        result = AsyncHttpCloseConnection(&wsio_instance->request);
...
int wsio_send(CONCRETE_IO_HANDLE ws_io, const void* buffer, size_t size, WSIO_MSG_TYPE message_type, ON_SEND_COMPLETE on_send_complete, void* callback_context)
{
    ...
    {
        WSIO_INSTANCE* wsio_instance = (WSIO_INSTANCE*)ws_io;
        result = add_pending_io(&wsio_instance->request,
            buffer,
            size,
            (int)message_type,
            on_send_complete,
...
void wsio_dowork(CONCRETE_IO_HANDLE ws_io)
{
    ...
    if (ws_io != NULL)
    {
		...
		case IO_STATE_ERROR:
			wsio_instance->io_state = IO_STATE_NOT_OPEN;
			wsio_instance->on_io_open_complete(wsio_instance->on_io_error_context, IO_OPEN_ERROR);
			break;
        default:
            if (!wsio_instance->request.pending_socket_io)
            {
...
int wsio_gethttpstatus(CONCRETE_IO_HANDLE socket_io)
{
    ...
    return (int)wsio_instance->request.dwStatusCode;
}
///////////////////////////////////////////////////////////////////////////////
