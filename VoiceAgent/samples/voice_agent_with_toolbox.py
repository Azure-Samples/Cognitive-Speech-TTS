"""Run the full Foundry Toolbox voice-agent lifecycle with microphone audio.

The sample creates or reuses a toolbox-enabled agent and opens a Voice Agent
session, prints user/agent transcripts and tool details, plays response audio,
and captures the persisted conversation id. Use the separate artifact downloader
for JSON and audio. The agent remains available.
"""
...
async def lifecycle(configured_agent_name: Optional[str] = None) -> None:
    ...
    tool = VoiceAgentToolboxTool(
        ...
        toolbox_version=os.getenv(
            "AZURE_VOICE_AGENTS_TOOLBOX_VERSION", "1"
        ),
    )
        ...
        if create_new:
            await client.agents.create_version(
                ...
                definition=VoiceAgentDefinition(
                    ...
                    model=model,
                    instructions=(
                        "Always use the toolbox search tool for factual questions. "
                        "After the tool returns, give one concise spoken answer."
                    ),
                    audio=audio_config,
                    output_modalities=[
