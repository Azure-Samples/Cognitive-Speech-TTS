...
- The Templates allowlist is `templates.config.json`. It detects the sibling
  ...
  `example2_finance_with_OTP_and_Officer_Search` sample projects and reloads
  their current `agent.json` files when **Reload** is selected.
- Template MCP readiness and publishing use the local files generated by
  ...
  Publishing creates or updates the configured project connection and creates
  a new `local-only-*` Agent in the portal's configured Foundry project. The
  prefix warns that its MCP connection depends on this machine's Dev Tunnel
  runtime and is not generally usable by another developer.
