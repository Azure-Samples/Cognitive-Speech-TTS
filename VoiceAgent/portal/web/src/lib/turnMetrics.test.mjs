...
test("sums completed replies with audible boundaries and explains every excluded response", () => {
  ...
  test("includes consecutive Agent replies in the latency table and sum", () => {
    const message = {
      id: "agent-follow-up",
      type: "assistant",
      ...
      metrics: {
        complete: true,
        startSource: "agent_playback_end",
        endToEndMs: 250,
        atomicPhases: [{ category: "inter_response_pause", durationMs: 250 }],
      },
    };

    assert.deepEqual(buildAllTurnLatencyTable([message]).rows.map((row) => row.id), [
      "agent-follow-up",
    ]);
    assert.deepEqual(buildLatencySumAnalysis([message]).includedTurns.map((turn) => turn.id), [
      "agent-follow-up",
    ]);
  });
