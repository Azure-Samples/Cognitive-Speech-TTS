...
test("uses the public SDK voice and voice_type output shape", () => {
  assert.deepEqual(buildVoiceOutput("alloy"), { voice: "alloy", voice_type: "openai" });
  assert.deepEqual(buildVoiceOutput("en-US-AvaNeural"), { voice: "en-US-AvaNeural", voice_type: "azure-standard" });
  const definition = buildVoiceDefinition({ model: "gpt-realtime", voice: "en-US-AvaNeural", inferenceMode: "model" });
  assert.equal(definition.audio.output.voice, "en-US-AvaNeural");
  assert.equal(definition.audio.output.voice_type, "azure-standard");
});

test("creates through the public version route with name only in the URL", () => {
  const body = buildCreateAgentBody({ name: "voice-test", model: "gpt-realtime", voice: "alloy", inferenceMode: "model" });
  const request = buildVersionCreateRequest({ apiVersion: "v1" }, body);
  assert.equal(request.path, "/agents/voice-test/versions?api-version=v1");
  assert.equal("name" in request.body, false);
  assert.equal(request.body.definition.audio.output.voice, "alloy");
  assert.equal(body.name, "voice-test"); // The authoring draft is not mutated.
  assert.throws(() => buildVersionCreateRequest({ apiVersion: "v1" }, {}), /name is required/);
});

test("normalizes the version-create response without losing version identity", () => {
  const version = { version: "3", definition: { kind: "voice" }, description: "Created version" };
  const resource = agentResourceFromVersion("voice-test", version);
  assert.deepEqual(resource, { name: "voice-test", versions: { latest: version } });
  assert.deepEqual(agentResourceFromVersion("voice-test", resource), resource);
});
...
test("prefers configured routing capabilities over stale shadow descriptions", () => {
  const name = "sample-hosted-agent";
...
test("requires an Azure output voice for hosted-agent wrappers", () => {
  ...
  assert.equal(body.definition.audio.output.voice, "en-US-AvaNeural");
  assert.equal(body.definition.audio.output.voice_type, "azure-standard");
});
