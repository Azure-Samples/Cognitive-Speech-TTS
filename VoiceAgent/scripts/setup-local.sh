...
if [[ "${missing_config}" == "0" ]]; then
  ...
  echo "next=${ROOT}/scripts/local-stack.sh restart"
else
  echo "local_setup=installed configuration=required"
  echo "rerun with: ./scripts/setup-local.sh --project-endpoint https://<account>.services.ai.azure.com/api/projects/<project>"
fi
